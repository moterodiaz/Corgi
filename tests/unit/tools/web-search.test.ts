    ...
    results: [
      {
        ...
        url:
          options.url ?? "https://example.test/venues/the-new-parkway-theater",
        content:
          options.content ??
          "A neighborhood cinema and cafe with evening events.",
...
describe("Tavily web search adapter", () => {
  ...
  it("accepts a 400-character event query and rejects 401 characters before fetch", async () => {
    const locality = "l".repeat(120);
    const eventSuffix =
      ` events in ${locality}` + "; event dates: 2026-08-01 through 2026-08-03";
  ...
  it.each([
    ...
    "rejects an unsafe Tavily result URL scheme as invalid_response: %s",
    async (unsafeUrl) => {
      const fetchMock = createFetchMock();
      fetchMock.mockResolvedValueOnce(
        tavilySuccess({
  ...
  it("retries HTTP 500 once and returns a successful second response", async () => {
    ...
    fetchMock
      ...
      .mockResolvedValueOnce(
        tavilySuccess({ requestId: "req_after_internal_error" }),
      );
    const sleep = vi.fn(() => Promise.resolve());
    const adapter = createTavilySearchAdapter({
      api_key: API_KEY,
  ...
  it.each([
    ...
    "expires the $name success cache at its documented TTL",
    async (testCase) => {
      const firstQuery =
        testCase.search === "venue" ? VENUE_QUERY : EVENT_QUERY;
      const fetchMock = createFetchMock();
      fetchMock
        .mockResolvedValueOnce(
          tavilySuccess({ query: firstQuery, requestId: "req_first" }),
        )
        .mockResolvedValueOnce(
          tavilySuccess({ query: firstQuery, requestId: "req_after_expiry" }),
        );
      let now = NOW;
      const adapter = createTavilySearchAdapter({
        api_key: API_KEY,
  ...
  it("redacts the API key and request data from translated fetch failures", async () => {
    ...
    fetchMock.mockRejectedValueOnce(
      new Error(
        `request failed with Authorization: Bearer ${API_KEY}; query=${privateQuery}`,
      ),
    );
    const adapter = createTavilySearchAdapter({
      api_key: API_KEY,
