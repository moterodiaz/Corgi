...
describe('feedback diff layer', () => {
  ...
  it('supports non-time hard constraints and preserves RSVPs absent from a partial update', async () => {
    const repo = await withCurrentPlan();
    const client = new QueueClaudeClient([{ kind: 'hard_constraint_change', patch: { venue: { name: 'Accessible Peak', source_tool: 'merge_search_venues', ref_id: 'peak-accessible' }, attendees: { sam: 'no' } }, requiresCandidateRefresh: true }]);
  ...
  it('applies a preference nudge without re-deriving unaffected fields', async () => {
    const repo = await withCurrentPlan();
    const client = new QueueClaudeClient([{ kind: 'preference_nudge', patch: { cost_tier: 'free', rationale: 'Budget-friendly.' }, requiresCandidateRefresh: true }]);
  ...
  it('leaves plan state untouched on full reject and requests a fresh synthesis', async () => {
    const repo = await withCurrentPlan();
    await repo.mergePersonProfile({ group_id: 'g1', person_id: 'sam', name: 'Sam', interests: [{ activity: 'climbing', confidence: 0.8, mention_count: 3, recency: Date.parse('2026-08-02T14:00:00-07:00') }], budget_signals: [], constraints: [], availability_mentions: [], updated_at: Date.now() });
    ...
    expect(await repo.getPersonProfiles('g1')).toMatchObject([{ person_id: 'sam', interests: [{ activity: 'climbing', mention_count: 3 }] }]);
  });
