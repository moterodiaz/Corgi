...
export class TavilySearchError extends Error {
  ...
  constructor(
    code: TavilySearchErrorCode,
    metadata: TavilySearchErrorMetadata = {},
  ) {
    ...
    this.retry_after_ms =
      ...
      metadata.retry_after_ms >= 0
        ...
        : undefined;
    this.provider_request_id = safeProviderRequestId(
...
const FetchSchema = z.custom<typeof globalThis.fetch>(
  (value) => typeof value === "function",
);
const NowSchema = z.custom<() => number>(
  (value) => typeof value === "function",
);
const SleepSchema = z.custom<TavilySleep>(
  (value) => typeof value === "function",
);
...
const TavilySearchAdapterConfigSchema = z
  .object({
    ...
    timeout_ms: z
      ...
      .min(1)
      .max(120_000)
      .default(DEFAULT_TAVILY_TIMEOUT_MS),
...
function parseOptions(
  options: TavilySearchOptions | undefined,
): TavilySearchOptions {
      ...
      (error: unknown) => {
        ...
        reject(
          error instanceof Error
            ? error
            : new Error("Asynchronous operation failed"),
        );
      },
    );
  });
...
class TavilySearchAdapterImplementation implements TavilySearchAdapter {
  ...
  async #search(
    ...
    if (request === undefined) {
      request = this.#request(sourceTool, query, maxResults).then(
        (evidence) => {
    ...
    const evidence =
      consumerSignal === undefined
        ? await request
        : await raceWithAbort(request, consumerSignal);
            ...
            if (retryDelay > 0) {
              await raceWithAbort(
                Promise.resolve().then(() =>
                  this.#config.sleep(retryDelay, controller.signal),
                ),
                controller.signal,
              );
            }
  ...
  async #fetch(
    ...
    try {
      ...
      if (
        signal.aborted ||
        (error instanceof TavilySearchError && error.code === "timeout")
      ) {
        throw new TavilySearchError("timeout");
      }
