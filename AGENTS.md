...
- **No secrets in code, logs, or error messages.** `ANTHROPIC_API_KEY`, `MERGE_API_KEY`, Photon credentials — env vars only, never interpolated into a log line, never returned in an API error body.
